...
const EMPTY_FORM: Omit<Department, 'id'> = { code: '', name: '', type: 'operation' };
...
export function DepartmentsTab() {
  ...
  const openAdd = () => {
    const nextCode = generateNextCode(items.map((d) => d.code).filter(Boolean), 'DEP');
    setForm({ ...EMPTY_FORM, code: nextCode });
    setModal({ open: true, mode: 'add', item: null });
  };
  const openEdit = (item: Department) => {
    setForm({ code: item.code, name: item.name, type: item.type });
    setModal({ open: true, mode: 'edit', item });
  };
  const closeModal = () => setModal({ open: false, mode: 'add', item: null });

  const handleSave = async () => {
    if (!form.code.trim()) { showToast('error', 'Department code is required'); return; }
            ...
            <div className="space-y-3">
              <div>
                ...
                <input value={form.code} onChange={(e) => setForm((f) => ({ ...f, code: e.target.value }))}
                  className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500" placeholder="e.g. DEP-001" />
              </div>
              <div>
