...
export function BranchesTab() {
  ...
  const openEdit = (item: Branch) => { setForm({ code: item.code, name: item.name, latitude: item.latitude, longitude: item.longitude, radius: item.radius ?? 50 }); setModal({ open: true, mode: 'edit', item }); };
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-600 uppercase tracking-wider">Geofence</th>
                ...
                <tr key={b.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-xs text-slate-500">
                    {b.latitude && b.longitude
                      ? <span className="inline-flex items-center gap-1 rounded-full bg-green-50 border border-green-200 px-2 py-0.5 text-green-700">📍 {b.radius ?? 50}m radius</span>
          ...
          <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
            ...
            <div className="space-y-3">
              ...
              <div className="pt-2 border-t border-slate-100">
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">📍 GPS Geofence (optional)</p>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-medium text-slate-600 mb-1">Latitude</label>
                    <input type="number" step="any" min="-90" max="90"
                      value={form.latitude ?? ''}
                      onChange={(e) => setForm((f) => ({ ...f, latitude: e.target.value === '' ? undefined : parseFloat(e.target.value) }))}
                      className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                      placeholder="e.g. 30.0522" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-slate-600 mb-1">Longitude</label>
                    <input type="number" step="any" min="-180" max="180"
                      value={form.longitude ?? ''}
                      onChange={(e) => setForm((f) => ({ ...f, longitude: e.target.value === '' ? undefined : parseFloat(e.target.value) }))}
                      className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                      placeholder="e.g. 31.2397" />
                  </div>
                </div>
                <div className="mt-3">
                  <label className="block text-xs font-medium text-slate-600 mb-1">Geofence Radius (meters)</label>
                  <input type="number" min="10" max="500"
                    value={form.radius ?? 50}
                    onChange={(e) => setForm((f) => ({ ...f, radius: parseInt(e.target.value) || 50 }))}
                    className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                    placeholder="50" />
                  <p className="mt-1 text-xs text-slate-400">Android app blocks check-in if employee is beyond this radius.</p>
                </div>
                {form.latitude && form.longitude && (
                  <div className="mt-2 rounded bg-blue-50 border border-blue-200 px-3 py-2 text-xs text-blue-700">
                    📍 {form.latitude.toFixed(6)}, {form.longitude.toFixed(6)} · {form.radius ?? 50}m geofence
                  </div>
                )}
              </div>
            </div>
